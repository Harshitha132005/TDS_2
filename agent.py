...
def get_llm():
    """Lazy initialization of LLM to allow server to start without API key."""
    global _llm
    if _llm is None:
...
def agent_node(state: AgentState):
    ...
    if prev_time is not None:
        ...
        if diff >= 180 or (offset != "0" and (cur_time - float(offset)) > 90):
            ...
            result = get_llm().invoke(state["messages"] + [fail_msg])
    ...
    result = get_llm().invoke(trimmed_messages)
